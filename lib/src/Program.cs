    ...
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            for (int i = 0; i < tdNode.Count / 5; i++)
            {
                ...
                if (htmlDoc != null)
                {
                    if (htmlDoc.DocumentNode.InnerText.Contains("limit"))
                        continue;
                    ...
                    if (image != null && image.Attributes["src"].Value.StartsWith("//"))
                        image.Attributes["src"].Value = image.Attributes["src"].Value.Replace("//", "https://");
